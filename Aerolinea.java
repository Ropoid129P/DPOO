...
public class Aerolinea
{
    ...
    {
        Vuelo buscado = null;
        for( Iterator<Vuelo> iterador = vuelos.iterator( ); iterador.hasNext( ) && buscado == null; )
        {
    ...
    {
        List<Tiquete> todosLosTiquetes = new ArrayList<Tiquete>( );
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
        ...
        // Revisar que el avión no esté ocupado en otro vuelo que se cruce con el nuevo vuelo
        int inicioNuevo = Ruta.getHoras( laRuta.getHoraSalida( ) ) * 60 + Ruta.getMinutos( laRuta.getHoraSalida( ) );
        int finNuevo = inicioNuevo + laRuta.getDuracion( );

        for( Vuelo vuelo : vuelos )
        {
            if( vuelo.getAvion( ) == elAvion && vuelo.getFecha( ).equals( fecha ) )
            {
                ...
                if( inicioNuevo < finOtro && inicioOtro < finNuevo )
                    throw new Exception( "El avión " + nombreAvion + " ya está ocupado en la ruta " + otraRuta.getCodigoRuta( ) + " el " + fecha );
            }
        }
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
        ...
        CalculadoraTarifas calculadora = getCalculadoraTarifas( fecha );

        return elVuelo.venderTiquetes( elCliente, calculadora, cantidad );
    }

    /**
     * Decide cuál calculadora de tarifas debe usarse según la fecha del vuelo.
     * 
     * La temporada baja va de enero a mayo y de septiembre a noviembre; el resto del año es temporada alta.
     * @param fecha La fecha del vuelo, expresada como una cadena de la forma YYYY-MM-DD
     * @return La calculadora de tarifas que corresponde a la temporada de la fecha
     */
    private CalculadoraTarifas getCalculadoraTarifas( String fecha )
    {
    ...
    public String consultarSaldoPendienteCliente( String identificadorCliente )
    {
        ...
        return "" + elCliente.calcularValorTotalTiquetes( );
    }
